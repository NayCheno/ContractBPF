...
WORK="$ROOT/build/memcached-natural-initramfs"
PKG_ROOT="$ROOT/build/memcached-pkgs/root"
DEB_DIR="$ROOT/build/memcached-pkgs/debs"
OUT="$ROOT/qemu/images/memcached-natural-initramfs.cpio.gz"
LOAD_BIN="${LOAD_BIN:-$ROOT/workloads/memcached/memcached_ascii_load}"
MEMORY_PRESSURE="${MEMORY_PRESSURE:-$ROOT/workloads/memory_pressure/pressure}"
...
for bin in "$LOAD_BIN" "$MEMORY_PRESSURE" "$SCX_BOOST" "$CONTRACTCTL" "$MM_LOADER"; do
    if [ ! -x "$bin" ]; then
...
if [ ! -r "$BPF_OUT/bad_demote.bpf.o" ]; then
    echo "ERROR: BPF object missing: $BPF_OUT/bad_demote.bpf.o" >&2
    echo "Run: make bpf" >&2
    exit 1
fi
...
rm -rf "$WORK"
mkdir -p "$WORK/bin" "$WORK/dev" "$WORK/etc/contractbpf" "$WORK/mnt/scratch" "$WORK/proc" \
    "$WORK/sys/kernel/debug" "$WORK/sys/fs/cgroup" "$WORK/tmp" \
    "$WORK/usr/local/bin" "$WORK/usr/local/lib/contractbpf" "$(dirname "$OUT")"
...
copy_bin "$MEMORY_PRESSURE" /usr/local/bin/memory_pressure
...
cp "$ROOT/bpf/contracts/service_a_paging_norevoke.yaml" "$WORK/etc/contractbpf/service_a_paging_norevoke.yaml"

cp "$ROOT/qemu/rootfs/memcached-natural-init.sh" "$WORK/init"
...
printf 'Built memcached natural initramfs: %s\n' "$OUT"
