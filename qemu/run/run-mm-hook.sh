...
INITRD="${CONTRACTBPF_MM_HOOK_INITRD:-$ROOT/qemu/images/mm-hook-initramfs.cpio.gz}"
QEMU_BIN="${QEMU_BIN:-qemu-system-x86_64}"
TIMEOUT="${QEMU_TIMEOUT:-60s}"
LOG_DIR="$ROOT/artifacts/logs"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOG="$LOG_DIR/${STAMP}-qemu-mm-hook.log"
...
if [ ! -f "$INITRD" ]; then
    "$ROOT/qemu/rootfs/build-mm-hook-rootfs.sh"
fi
...
printf 'PASS: ContractBPF MM hook prototype passed in %s\n' "$LOG"
