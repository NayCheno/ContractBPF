...
INITRD="${CONTRACTBPF_SCHED_INITRD:-$ROOT/qemu/images/sched-ext-initramfs.cpio.gz}"
QEMU_BIN="${QEMU_BIN:-qemu-system-x86_64}"
TIMEOUT="${QEMU_TIMEOUT:-90s}"
LOG_DIR="$ROOT/artifacts/logs"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOG="$LOG_DIR/${STAMP}-qemu-sched-ext.log"
...
if [ ! -f "$INITRD" ]; then
    "$ROOT/qemu/rootfs/build-sched-rootfs.sh"
fi
...
printf 'PASS: sched_ext baseline loaded and unloaded in %s\n' "$LOG"
