...
INITRD="${CONTRACTBPF_INITRD:-$ROOT/qemu/images/initramfs.cpio.gz}"
QEMU_BIN="${QEMU_BIN:-qemu-system-x86_64}"
TIMEOUT="${QEMU_TIMEOUT:-60s}"
LOG_DIR="$ROOT/artifacts/logs"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOG="$LOG_DIR/${STAMP}-qemu-smoke.log"
...
if [ ! -f "$INITRD" ]; then
    echo "ERROR: initramfs missing: $INITRD" >&2
    exit 1
fi
...
if grep -q 'CONTRACTBPF_BOOT_OK' "$LOG"; then
    printf 'PASS: CONTRACTBPF_BOOT_OK found in %s\n' "$LOG"
    exit 0
fi

echo "FAIL: CONTRACTBPF_BOOT_OK missing; log: $LOG" >&2
tail -n 80 "$LOG" >&2
exit 1
