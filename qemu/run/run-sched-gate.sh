...
INITRD="${CONTRACTBPF_SCHED_GATE_INITRD:-$ROOT/qemu/images/sched-gate-initramfs.cpio.gz}"
QEMU_BIN="${QEMU_BIN:-qemu-system-x86_64}"
TIMEOUT="${QEMU_TIMEOUT:-90s}"
LOG_DIR="$ROOT/artifacts/logs"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOG="$LOG_DIR/${STAMP}-qemu-sched-gate.log"
...
if [ ! -f "$INITRD" ]; then
    "$ROOT/qemu/rootfs/build-sched-gate-rootfs.sh"
fi
...
printf 'PASS: sched_ext ContractBPF gate throttled boost in %s\n' "$LOG"
