...
LOG="$LOG_DIR/${STAMP}-qemu-no-violation-overhead.log"
...
CMD=(timeout "$TIMEOUT" "$QEMU_BIN" "${KVM_ARGS[@]}" \
    ...
    -kernel "$KERNEL" \
    -initrd "$INITRD" \
    -append "console=ttyS0 panic=-1 nokaslr contractbpf.no_violation_overhead=1" \
    -nographic \
    -no-reboot)
...
for marker in \
    CONTRACTBPF_BOOT_OK \
    CONTRACTBPF_NO_VIOLATION_OVERHEAD_BEGIN \
    CONTRACTBPF_NO_VIOLATION_OVERHEAD_OK; do
...
printf 'PASS: ContractBPF no-violation overhead probe passed in %s\n' "$LOG"
