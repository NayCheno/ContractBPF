...
INITRD="${CONTRACTBPF_SELFTEST_INITRD:-$ROOT/qemu/images/contractbpf-selftest-initramfs.cpio.gz}"
QEMU_BIN="${QEMU_BIN:-qemu-system-x86_64}"
TIMEOUT="${QEMU_TIMEOUT:-60s}"
LOG_DIR="$ROOT/artifacts/logs"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOG="$LOG_DIR/${STAMP}-qemu-contractbpf-kselftest.log"
...
if [ ! -f "$INITRD" ]; then
    "$ROOT/qemu/rootfs/build-contractbpf-selftest-rootfs.sh"
fi
...
printf 'PASS: ContractBPF kselftest passed in %s\n' "$LOG"
