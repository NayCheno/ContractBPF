...
INITRD="${CONTRACTBPF_MATRIX_INITRD:-$ROOT/qemu/images/experiment-matrix-initramfs.cpio.gz}"
QEMU_BIN="${QEMU_BIN:-qemu-system-x86_64}"
TIMEOUT="${QEMU_TIMEOUT:-180s}"
LOG_DIR="$ROOT/artifacts/logs"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOG="$LOG_DIR/${STAMP}-qemu-experiment-matrix.log"
...
if [ ! -f "$INITRD" ]; then
    "$ROOT/qemu/rootfs/build-experiment-matrix-rootfs.sh"
fi
...
for marker in CONTRACTBPF_BOOT_OK CONTRACTBPF_EXPERIMENT_MATRIX_OK; do
...
printf 'PASS: ContractBPF experiment matrix passed in %s\n' "$LOG"
