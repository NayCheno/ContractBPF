...
We implement \sys on Linux using sched\_ext and a conservative PageFlex-style paging decision hook. The current artifact validates the mechanism in QEMU: controlled scheduler-paging experiments reproduce the feedback loop, and a memcached companion path demonstrates the same artifact on a real networked service. These QEMU results are correctness and reproducibility evidence, not production performance claims.
\end{abstract}
...
This section summarizes the evidence currently produced by the artifact. All
experiments run in QEMU on the pinned Linux kernel, so the absolute latency and
throughput numbers are not production performance results.

\subsection{Workloads}

The main matrix uses a controlled phase-changing service that emits latency
samples while exercising scheduler and paging effects. The artifact also
includes a real memcached path: QEMU boots two memcached instances, drives them
with an ASCII protocol client, and records tenant latency under controlled and
natural memory-pressure scenarios.
...
The current QEMU artifact shows four results:

\begin{enumerate}[leftmargin=*]
    \item The kernel patch series builds, QEMU boots, sched\_ext loads and unloads, and kselftests exercise token, ledger, degrade, MM hook, and cross-subsystem paths.
    \item The natural QEMU memory-pressure scenario records page demotion, refaults or major faults, and scheduler queue delay without controlled counter injection.
    \item \sys detects the cross-subsystem violation and revokes the harmful demote\_page effect while preserving scheduler dispatch.
    \item The QEMU memcached natural-bars proxy records a high-P99 conflict window and a lower-P99 recovery window, and the native non-QEMU VM run records same-load P5/P6 bars for the current final-evidence gate.
\end{enumerate}
...
The mechanisms are kernel mechanisms, but the failure and evaluation target networked services. The paper argues that programmable resource policies are becoming part of cloud service management. A strong NSDI submission must therefore emphasize multi-tenant service stability, tail latency, workload phases, and operational usefulness.

\subsection{Limitations}

\sys currently targets sched\_ext and paging. It does not handle all BPF program types, does not replace the verifier, and does not prove global optimality of resource policies. Threshold selection and scope mapping require careful configuration. The current evaluation includes QEMU correctness evidence and native non-QEMU VM P5/P6 bars, but it must not be read as bare-metal or production performance evidence.
